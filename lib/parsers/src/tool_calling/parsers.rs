...
pub fn get_tool_parser_map() -> &'static HashMap<&'static str, ToolCallConfig> {
    PARSER_MAP.get_or_init(|| {
        ...
        map.insert("qwen25", ToolCallConfig::qwen25()); // hermes <tool_call> format, but drops unterminated calls (no EOF recovery)
        map
    })
}
...
async fn detect_and_parse_tool_call_with_recovery_options(
    ...
    let recovery_config = match &base.parser_config {
        ParserConfig::Json(c) => {
            ...
            c.allow_eof_recovery = !c.disable_eof_recovery;
            ParserConfig::Json(c)
        }
        ParserConfig::Xml(c) => {
            let mut c = c.clone();
